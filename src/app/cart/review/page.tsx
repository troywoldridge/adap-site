...
// Next.js runtime hints for this Server Component page
...
async function loadCart() {
  ...
  // Load any cart not "closed" so pending still shows
  const [cart] =
    (await db
      .select({
        id: carts.id,
  ...
  const lines = rows.map((r) => {
    ...
    const totalCents = Number(
      r.lineTotalCents ?? (Number.isFinite(unitCents) ? unitCents * qty : 0)
    );
...
export default async function ReviewCartPage() {
  ...
  const tax = 0; // wire your tax calc here if needed
