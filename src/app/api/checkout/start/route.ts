...
function pickCfId(r: ProductAssetRow): string | null {
  const ids = [r.cf_image_1_id, r.cf_image_2_id, r.cf_image_3_id, r.cf_image_4_id, r.cf_image_id];
...
export async function POST() {
  try {
    ...
    if (!sid) {
      return NextResponse.json({ ok: false, error: "No session/cart." }, { status: 400 });
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: { cartId: String(cart.id), sid }, // ✅ camelCase to match webhook
