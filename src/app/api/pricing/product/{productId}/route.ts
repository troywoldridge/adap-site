...
export async function getPricingRows(productId: number, hashes: string[]): Promise<PricingRow[]> {
  ...
  return rows.map((row) => ({
    ...
    rawDimensions: row.rawDimensions ?? undefined
  }));
}
...
// ✅ Get all pricing rows for a product
export async function getPricingByProductId(productId: number): Promise<PricingRow[]> {
  const rows = await db
    .select()
    .from(pricing)
    .where(eq(pricing.product, productId));
