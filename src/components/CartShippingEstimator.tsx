...
import { useCartShippingEstimate } from '@/hooks/useShippingEstimate';
import type { CartLine } from '@/lib/cart';

// If you have a Cart context, replace this with your hook and remove the `lines` prop.
type Props = {
  // Optional override if you don't have a cart hook available at this layer:
  lines?: CartLine[];
  // Optional defaults:
...
export default function CartShippingEstimator({
  ...
  async function onSubmit(e: React.FormEvent) {
    e.preventDefault();
    await estimate({ shipCountry: country, shipState: state, shipZip: zip, lines });
  }

  return (
    <div className="space-y-4">
      <h3 className="text-lg font-semibold">Estimate Shipping (Cart)</h3>
      ...
      {!!rates?.length && (
        <ul className="divide-y rounded border">
          {rates.map((r) => (
            <li key={`${r.serviceCode}-${r.amount}`} className="p-3 flex items-center justify-between">
              <div>
